...
public class Cowitem extends MarketItem {
	...
	public Cowitem(int x, int y, int price) {
		super(x, y, price);
		i = Images.cow1.getScaledCopy(size,size);
	}
	...
	public void render(Graphics g) {
		...
		if(mouseOver()) {
			...
			displaytext( g, "Cow", "buy", "Produces milk", Player.cows, price);	
		}
		else {
...
//				Game.worldManager.animals.add(new Cow(Game.tileSize*1,Game.tileSize*1))type name = new type();
					...
					for(int i = startfencei + 1;i<=endfencei - 1;i++) 
					{
						for(int j = startfencej + 1; j <= endfencej - 1;j++) 
						{
							...
							{
								Game.worldManager.addAnimal(i, j, 1);
								purchased = true;
							}
					...
					Player.cows+=1;
					Player.coins-=price;
					Market.explosionclusters.add(new Explosioncluster("cow",10,x,y));
