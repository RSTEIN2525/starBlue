...
public class Chickenitem extends MarketItem {
	...
	public Chickenitem(int x, int y, int price) {
		super(x, y, price);
		i = Images.chicken1.getScaledCopy(size,size);
	}
	...
	public void render(Graphics g) {
		...
		if(mouseOver()) {
			...
			displaytext( g, "Chicken", "buy", "Produces eggs", Player.chickens, price);	
		}
		else {
...
//				Game.worldManager.animals.add(new Cow(Game.tileSize*1,Game.tileSize*1))type name = new type();
					...
					for(int i = startfencei + 1;i<=endfencei - 1;i++) 
					{
						for(int j = startfencej + 1; j <= endfencej - 1;j++) 
						{
							...
							{
								Game.worldManager.addAnimal(i, j, 3);
								purchased = true;
							}
					...
					Player.chickens+=1;
					Player.coins-=price;
					Market.explosionclusters.add(new Explosioncluster("chicken",10,x,y));
