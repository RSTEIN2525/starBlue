...
public class CopperOre extends MarketItem{

	public Boolean buyable = false;
	Image i;
	
	public CopperOre(int x, int y, int price) {
		super(x, y, price);
		i = Images.copperore.getScaledCopy(size,size);
	}
...
public void render(Graphics g) {
		...
		if(mouseOver()) {
			...
			displaytext( g, "Copper Ore", "sell", "Rare and Valuable Ore", Player.copper, price);
...
void transaction(GameContainer gc) {
	...
	if(mouseOver() && input.isMousePressed(Input.MOUSE_LEFT_BUTTON)) {
		...
		if(Player.copper>0) {
			...
			Player.coins+=price;
			Game.copperSold ++;
			Market.explosionclusters.add(new Explosioncluster("copperfragment",10,x,y));
		}	
