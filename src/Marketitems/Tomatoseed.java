...
public class Tomatoseed extends MarketItem {

	public Boolean buyable = true;
	Image i;
	
	public Tomatoseed(int x, int y, int price) {
		super(x, y, price);
		i = Images.tomatoSeeds.getScaledCopy(size,size);
	}
	...
	public void render(Graphics g) {
		...
		if(mouseOver()) {
			...
			displaytext( g, "Tomato seeds", "buy", "turns into tomatoes", Player.tomatoSeeds, price);	
		}
		else {
	...
	if(mouseOver() && input.isMousePressed(Input.MOUSE_LEFT_BUTTON)) {
		...
		if(buyable) {
			...
			if(Player.coins>=price) {
				...
				Player.tomatoSeeds+=1;
				Player.coins-=price;
				Market.explosionclusters.add(new Explosioncluster("tomatoseed",10,x,y));
