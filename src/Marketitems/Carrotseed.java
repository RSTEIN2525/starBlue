...
public class Carrotseed extends MarketItem {

	public Boolean buyable = true;
	Image i;
	
	public Carrotseed(int x, int y, int price) {
		super(x, y, price);
		i = Images.carrotSeeds.getScaledCopy(size,size);
	}
	...
	public void render(Graphics g) {
		...
		if(mouseOver()) {
			...
			displaytext( g, "Carrot seeds", "buy", "turns into Carrots", Player.carrotSeeds, price);	
		}
		else {
	...
	if(mouseOver() && input.isMousePressed(Input.MOUSE_LEFT_BUTTON)) {
		...
		if(buyable) {
			...
			if(Player.coins>=price) {
				...
				Player.carrotSeeds+=1;
				Player.coins-=price;
				Market.explosionclusters.add(new Explosioncluster("carrotseed",10,x,y));
			}
