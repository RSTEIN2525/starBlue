...
public class Corn extends MarketItem {

	public Boolean buyable = false;
	Image i;
	
	public Corn(int x, int y, int price) {
		super(x, y, price);
		i = Images.cornShop.getScaledCopy(size,size);
	}
	...
	public void render(Graphics g) {
		...
		if(mouseOver()) {
			...
			displaytext( g, "Corn", "sell", "corn is corn", Player.corn, price);
...
void transaction(GameContainer gc) {
	...
	if(mouseOver() && input.isMousePressed(Input.MOUSE_LEFT_BUTTON)) {
		...
		if(Player.corn>0) {
			...
			Player.coins+=price;
			Game.cornSold ++;
			Market.explosionclusters.add(new Explosioncluster("corn",10,x,y));
		}	
